...
if !l9#guardScriptLoading(expand('<sfile>:p'), 0, 0, ['has("python3")'])
  finish
endif
...
function l9#async#execute(key, cmd, cwd, input, appends)
  ...
  python3 asyncer.execute('a:key', 'a:cmd', 'a:cwd', 'a:input', 'a:appends')
endfunction
...
function l9#async#read(key)
  call s:checkKey(a:key)
  redir => result
  silent python3 asyncer.print_output('a:key')
  redir END
...
function l9#async#listWorkers()
  redir => result
  silent python3 asyncer.print_worker_keys()
  redir END
  return split(result, "\n")
endfunction
...
function l9#async#listActiveWorkers()
  redir => result
  silent python3 asyncer.print_active_worker_keys()
  redir END
  return split(result, "\n")
endfunction
